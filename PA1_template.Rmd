...
```{r transform_tbl_df, echo=TRUE, message=FALSE, warning=FALSE}
library(dplyr)
...
```{r sort_avg, echo=TRUE}
steps_by_interval_sorted <- arrange(steps_by_interval, desc(average))
max_interval <- steps_by_interval_sorted$interval[1]
```

The identifier of the interval that has the maximum average number of steps is **`r max_interval`**.

## 3. Imputing missing values

**(1) Calculate and report the total number of missing values in the dataset** (i.e. the total number of rows with `NAs`)

```{r compute_total_na, echo=TRUE}
...
The mean and median total number of steps taken per day are **`r mean_1`** and **`r median_2`**, respectively, which are both greater than the estimates from the first part of the assignment. 
...
```{r panel_plot, echo=TRUE, warning=FALSE}
library(ggplot2)
